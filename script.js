...
const PRICES = { silicone: 19.90, steel: 29.90, bundle: 44.90 };
const TITLES = {
  de: {
...
// PayPal live schalten (nur mit Live-Client-ID im HTML!)
...
function setVariant(variant) {
  ...
  document
    .querySelectorAll(".variant input")
    .forEach((r) => (r.checked = r.value === v));
  document
    .querySelectorAll(".product-title")
    .forEach((el) => (el.textContent = TITLES[lang][v]));
  document
    .querySelectorAll(".price")
    .forEach((el) => (el.textContent = PRICES[v].toFixed(2) + " €"));
  document
    .querySelectorAll(".paypal-guard")
    .forEach((el) => (el.style.display = window.PAYPAL_LIVE ? "none" : "block"));
}

// ✅ Robust: wiederholt versuchen, bis das SDK geladen ist
function renderPayPalButtons() {
  if (!window.PAYPAL_LIVE) return;
  const container = document.getElementById("paypal-button-container");
  if (!container) return;

  // Mehrfach-Render vermeiden
  if (window.__ppMounted) return;
  ...
  if (window.paypal && paypal.Buttons) {
    ...
    // SDK noch nicht ready → in 300ms erneut probieren
...
// Lead-Formular – öffnet Mail-Client (unverändert)
function onLeadSubmit(e) {
  ...
  const subject = encodeURIComponent(
    lang === "de" ? "Vormerkung Ice Roller" : "Ice Roller Reservation"
  );
